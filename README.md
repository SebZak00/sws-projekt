...
1. Sklonuj repozytorium do wybranego folderu:
   git clone https://github.com/TWOJ_NICK/sws-projekt.git
   cd sws-projekt
...
3. **Cache'owanie aplikacji (Wydajność):**
   Aby drastycznie przyspieszyć działanie systemu, należy wygenerować pliki cache dla konfiguracji, ścieżek routingu oraz widoków:
   php artisan config:cache
   php artisan route:cache
   php artisan view:cache

4. **Kompilacja i wersjonowanie zasobów Frontendowych:**
   Zasoby statyczne (CSS/JS) należy zminifikować do wersji produkcyjnej, aby zredukować ich rozmiar:
   npm install
   npm run build
